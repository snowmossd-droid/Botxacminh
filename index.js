...
const { handleMusicCommand, handleStopCommand } = require('./music');
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildVoiceStates, // can cho tinh nang phat nhac
...
client.on('messageCreate', async (message) => {
  ...
  if (content.toLowerCase().startsWith('!music ')) {
    const query = content.slice('!music '.length).trim();
    await handleMusicCommand(message, query);
    return;
  }
...
client.on('messageCreate', async (message) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '!music <ten bai hat>', value: 'Tim va phat nhac trong kenh voice ban dang o. Vi du: `!music shape of you`' },
      { name: '!stop', value: 'Dung nhac va cho bot roi kenh voice.' },
